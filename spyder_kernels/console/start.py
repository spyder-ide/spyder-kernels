...
from spyder_kernels.utils.mpl import MPL_BACKENDS_FROM_SPYDER, INLINE_FIGURE_FORMATS
...
def kernel_config():
    ...
    spy_cfg.IPKernelApp.exec_lines.append("get_ipython().kernel._load_wurlitzer()")
        ...
        if pylab_o == "True" and backend_o is not None:
            ...
            if mpl_backend == "inline":
                ...
                if resolution_o is not None:
                    spy_cfg.InlineBackend.rc["figure.dpi"] = float(resolution_o)
...
def varexp(line):
    ...
    __items__ = getattr(pyplot, funcname[2:])(ip.kernel._get_current_namespace()[name])
    pyplot.show()
