...
class IPdbKernel(BaseKernelMixIn, MetaKernel):
    ...
    # IMPORTANT: The kernelspec generated by Metakernel is only meant
    # for testing!
    kernel_json = {
    ...
    def __init__(self, *args, testing=False, **kwargs):
        ...
        if os.environ.get('SPY_TEST_IPDB_KERNEL') is not None:
            self.testing = True

        # Create a kernel client connected to the console kernel
        if not self.testing:
            console_client = self._create_console_client()
        else:
            console_client = self._create_dummy_client()

        # Create Pdb proxy
        self.debugger = PdbProxy(parent=self, kernel_client=console_client)
        ...
        # Remove unneeded magics that come by default with Metakernel
        ...
        if not is_ready:
            if not self.testing:
                # TODO: Add here a message printed in the console
                # kernel saying that IPdb kernel failed to start
                pass
            else:
    ...
    def _create_dummy_client(self):
        """
        Create a dummy console kernel client.
